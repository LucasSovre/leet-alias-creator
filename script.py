# -*- coding: utf-8 -*-
file= open('badwordList.txt' , "r", encoding='utf8')
liste = []
variant =[]
...
for item in liste : 
    a=item+'e' #we add an a feminine form for each term
    if a.endswith('ee')== False and a.endswith('éee') == False and a.endswith('èee') ==False :
        variant.append(a)
    #we replace each "c" by "k" if it's pronounce [k]
    b=list(item)
    i=0
    for letter in b :
        if i+1 < len(b) :
            if letter == 'c' and b[i+1] != 'h': #exclued all 'c' pronounce [ʃ]
                b[i] = 'k'
            i=i+1
    if  "".join(b) != item: 
        variant.append("".join(b))
...
file.close()
modifiedFile= open('modifiedBadWordList1.txt','w',encoding='utf8')
